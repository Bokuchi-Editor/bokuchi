...
| M-163 | HTML export code block background is uniform *(regression)* | In a dark theme, write a fenced code block with a syntax-highlighted language (e.g. ` ```js `), export to HTML, and open the file in a browser | The inner code area and the outer `<pre>` frame share the exact same background color — no lighter inner rectangle. The block looks identical to the in-app preview |
| M-166 | External (https) image rendering *(regression — must test in a packaged build)* | In a **packaged build** (not `tauri dev`), write `![alt](https://raw.githubusercontent.com/.../some.png)` referencing a real public image, and view the preview | The remote image loads and displays. **Test in a build only:** `tauri dev` bypasses the tauri.conf CSP so remote images always load there; the CSP (which gates `img-src`) is enforced only in packaged builds |
| M-167 | Bold/italic next to CJK text *(#400)* | Write Japanese prose where the emphasis delimiter touches CJK punctuation/letters, e.g. `これは**「重要」**な話。` and `単なる「テキスト」から、**「アシスタント」**へと進化します。` and `「あ」*強調*へ`. Also write `` `**コード**` `` (inline code) and a fenced block containing `**コード**`. View the preview and export to HTML | The `**…**` / `*…*` spans render as bold/italic in both preview and exported HTML — the `*` markers do NOT show as literal text. Asterisks inside inline code and fenced code blocks stay literal (not bolded). No stray invisible characters are introduced |
...
| M-93 | Relative-path image rendering                            | Open a Marp document that references a sibling image (e.g. `![](./image.png)`)                                                                        | The image is rendered correctly inside slides (loaded from disk via the Tauri asset protocol `asset://`, NOT base64-inlined)                  |
...
| M-115 | Outline display mode                 | Switch Settings > Interface > Outline Display Mode between Always visible / Overlay / Off, then open the outline panel | Always visible reserves space alongside the editor; Overlay floats above the editor; Off hides the outline entirely and dims the header button |
| M-168 | Outline Off persists across restart  | Set Outline Display Mode to Off, then restart the app                                                              | The outline stays hidden at launch and Settings still shows Off (unlike the old Ctrl+Shift+O hide, which reset to visible on restart) |
| M-169 | Header button restores outline from Off | With Outline set to Off, click the header outline button (or press Ctrl+Shift+O)                                | The outline turns back on using the previously chosen style (Always visible or Overlay); Settings switches back from Off to that style |
| M-170 | Outline button turns it off (persisted) | With Always visible outline shown, click the header outline button                                              | The panel hides and Settings switches to Off; it stays off after restart. In Overlay mode the button only opens/closes the drawer — Off is set from Settings only |
