...
describe('markdownToolbarActions', () => {
  describe('buildWrapEdit', () => {
    ...
    // T-MTA-16: placeholder selection offsets from a mid-line caret — the
    // column arithmetic must be relative to the caret, not the line start.
    it('T-MTA-16: computes placeholder selection from a mid-line cursor', () => {
      const midLine = { startLineNumber: 2, startColumn: 5, endLineNumber: 2, endColumn: 5 };
      const { newSelection } = buildWrapEdit(midLine, '', '**', '**', 'bold');
      expect(newSelection).toEqual({
        startLineNumber: 2,
        startColumn: 7, // 5 + '**'.length
        endLineNumber: 2,
        endColumn: 11, // 7 + 'bold'.length
      });
    });
  });
