...
describe('useFileChangeDetection', () => {
  ...
  // T-FCD-12: dialog reload delegates content sync to reloadTabContent.
  // The Monaco model sync used to live here, but it was moved into
  // reloadTabContent (via editorSync.syncModelForTab) so that every reload
  // path in the app — dialog reload, save-time conflict reload — gets the
  // model sync for free. Tests for the model sync itself live in
  // editorSync.test.ts.
  it('T-FCD-12: onReload calls reloadTabContent with the freshly read content', async () => {
    const { desktopApi } = await import('../../api/desktopApi');
